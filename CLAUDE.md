...
**Symbol lookup** (`explore_find.py`): the `explore.find` method turns a symbol name, a node type, and a list of `SearchScope` ancestors into `explore.describe` paths — letting a client resolve the symbol under the cursor without searching its own cache. Generic: each driver declares *where* each kind of node lives in its tree as `BaseDriver.FIND_PATHS` path templates (`"*"` = one level of children, any other segment = a literal group name), and `walk_find` expands them through `explore_list`, pruning each wildcard level by the scopes. Scopes sharing a `type` are alternatives, scopes of different types compound. `MAX_LIST_CALLS` caps the fan-out of an under-scoped search.

`CachingDriver.explore_find` resolves in four passes, cheapest first: (1) the **find cache** — `ConnectionCache.get_find`, keyed by `find_key(node_type, name, scopes)`; (2) `walk_find` over levels *already cached*, via the cache-only `_cached_list`; (3) the wrapped driver's own `explore_find`, for a driver that can answer in one catalog query; (4) `walk_find` over the live `explore_list`, which runs on `CachingDriver` rather than the wrapped driver so it populates the cache for pass 2. Only pass 3 is written to the find cache — it is the only pass that leaves nothing behind for the others, since pass 4's walk caches every level it lists and so is served by pass 2 next time. The find cache is persisted in the same JSON file as `list`/`describe` (so it survives restarts, which is the whole point: a driver's catalog lookup populates nothing the walk can read), and because it is keyed by *search* rather than by path it cannot be evicted per-prefix — any `reset` clears it whole, in both directions (a find may now point into the reset subtree, or newly match something created there). A cache-only walk that *completes* is authoritative, empty result included — only one that runs off the end of the cache (`_CacheMiss`) or its call budget falls through. Pass 1 runs only for a node type `explore_find.declares` finds in `FIND_PATHS`: with no template `walk_find` returns empty *without reading the cache*, which would otherwise wrongly settle a search the driver's own lookup covers. It needs no guard against a cold cache — the walk short-circuits on its first `children()` call (~2µs). A driver overriding `explore_find` raises `FindNotSupported` for the node types its query does not cover; Oracle's override (`drivers/oracle/queries.py`, the `fetch_find_*` functions) resolves table/view/column/index across every schema in one dictionary query, repeating `fetch_schemas`' non-system owner filter as a subquery so a find never returns a path the tree does not contain, and leaves `schema` to the walker.
